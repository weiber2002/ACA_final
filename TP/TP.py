...
class My_LMHEAD(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # print("local_logits shape is " ,local_logits.shape)
        logits_list = [torch.empty_like(local_logits) for _ in range(self.world_size)]

        # Run all_gather
        dist.all_gather(logits_list, local_logits)
        logits_torch = torch.cat(logits_list, dim=-1)
        # print("logits_torch shape " , logits_torch.shape)
        return logits_torch
...
def main():
    ...
    my_max_new_tokens = 4000 #change max new tokens
    my_temperature = 0.7 #change when use temperature
    ...
    # initital distributed
    dist.init_process_group(backend="nccl", init_method="env://")
    torch.cuda.set_device(local_rank)
    device = torch.device(f"cuda:{local_rank}")
    
    
    # Load model by huggingface
    ...
    #inputs = tokenizer("Please tell me the history of Arsenal.", return_tensors="pt").to(local_rank)
    ...
    # replace DecoderLayer and My_LMHEAD
    ...
    # inputs = tokenizer("Please explain the history of Arsenal.", return_tensors="pt").to(local_rank)
    summarization_length = inputs["input_ids"].shape[-1]
    if(USE_TEMPERATURE_SAMPLING == True):
        ...
        if rank ==0 :
            ...
            print("Summarization Length = " ,summarization_length)
            print("Generation Length = " ,generated[0].shape[-1]- summarization_length)
            print("Total Length = " ,generated[0].shape[-1])
            print("Total Throughput is " , ( generated[0].shape[-1] /elapsed_time_ms)*1000 )
        ...
        # 5) inference: inside MyCustomMLP.forward you already all_reduce partials
        with torch.no_grad():
            outputs = model.generate(**inputs, max_new_tokens=my_max_new_tokens , temperature=0.0, do_sample=False ,  eos_token_id=tokenizer.eos_token_id, pad_token_id=tokenizer.pad_token_id  )
        torch.cuda.synchronize()
        end_event.record()
        elapsed_time_ms = start_event.elapsed_time(end_event)
        ...
        if rank ==0 :
            ...
            print(f"Inference time: {elapsed_time_ms:.3f} ms")
            print("Summarization Length = " ,summarization_length)
            print("Generation Length = " ,outputs[0].shape[-1]- summarization_length)
            print("Total Length = " ,outputs[0].shape[-1])
            print("Total Throughput is " , ( outputs[0].shape[-1] /elapsed_time_ms)*1000 )
