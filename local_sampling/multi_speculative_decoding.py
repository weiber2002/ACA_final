...
def multi_draft_speculative_generate(
    ...
    #     target = DataParallel(target, device_ids=list(range(torch.cuda.device_count())))
    # DataParallel 在每次 forward 时会先把整个模型的输入张量切分（scatter）到每张卡，然后再把所有卡算出的输出收回（gather）到主卡。scatter/gather 本身就会产生跨 GPU 的点对点通信，并且有额外的 CUDA kernel 调动，开销并不小。
    # buffers
    ...
    while current_pos < total_len:
        ...
        # seq_len = current_pos + g
        # batch_ids = batched_ids[:, :seq_len]
        # parts = batch_ids.chunk(2, dim=0)  # 例如 B=4 → parts[0].shape=(2,L), parts[1].shape=(2,L)
        # outs = []
        # for dev_idx, part in enumerate(parts):
        #     dev = f'cuda:{dev_idx}'
        #     outs.append(
        #         target(
        #             input_ids=part.to(dev),
        #             use_cache=use_cache,
        #         )
        #     )
        # DataParallel 在每次 forward 时会先把整个模型的输入张量切分（scatter）到每张卡，然后再把所有卡算出的输出收回（gather）到主卡。scatter/gather 本身就会产生跨 GPU 的点对点通信，并且有额外的 CUDA kernel 调动，开销并不小。
        # # 3) 把兩張卡算出的 logits 拼回來
        # logits = torch.cat([o.logits for o in outs], dim=0)  # shape = (B, L, vocab)
        # logits_batch = logits[..., current_pos-1 : current_pos+g-1, :]
