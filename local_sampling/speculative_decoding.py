...
def speculative_generate(
    ...
    input_ids = torch.full((1, total_len), pad_token_id, dtype=torch.long, device=target.device) # torch.tensor([[pad_token_id, pad_token_id, ..., pad_token_id]])
    input_ids[0, :prompt_len] = torch.tensor(inputs, dtype=torch.long, device=target.device)     # put the input sequence in the first part of the tensor
